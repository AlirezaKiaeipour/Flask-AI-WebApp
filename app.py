...
@app.route("/admin-setting-change", methods=["GET", "POST"])
def admin_setting_change():
    if request.method == "GET":
        return make_response(render_template("admin_setting_change.html"))
    elif request.method == "POST":
        user = request.form["username_update_admin_change"]
        email = request.form["email_update_admin_change"]
        user = database.update_role(user, email)
        if user is not None:
